...
import (
	...
	logPb "github.com/GoogleCloudPlatform/microservices-demo/src/checkoutservice/log"
...
type checkoutService struct {
	...
	orderlogSvcAddr string
	orderlogSvcConn *grpc.ClientConn
}
...
func main() {
	...
	mustMapEnv(&svc.orderlogSvcAddr, "ORDERLOG_SERVICE_ADDR")
	...
	mustConnGRPC(ctx, &svc.orderlogSvcConn, svc.orderlogSvcAddr)
...
func (cs *checkoutService) PlaceOrder(ctx context.Context, req *pb.PlaceOrderRequest) (*pb.PlaceOrderResponse, error) {
    ...
    orderEncoded, err := json.Marshal(orderResult)
    if err != nil {
        log.Warning("marshalling of order failed")
    } else {
        // for debugging
        fmt.Println(string(orderEncoded))

        // logging the order
        _, err := logPb.NewLoggerClient(cs.orderlogSvcConn).
            Log(ctx, &logPb.Entry{Content: fmt.Sprintf("new order finished: %s", string(orderEncoded))})
        if err != nil {
            log.Warning("logging of order failed")
        }
    }
