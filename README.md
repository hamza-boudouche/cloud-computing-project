...
`kubectl` command that fetches and follows its logs. If everything's working as it
...
by defining the version of the API it's referecing, in this case it's `v1`, which
...
As explained in the lab, we are going to deploy the load generator separately from the
